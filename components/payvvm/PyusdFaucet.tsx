...
import { Droplet, CheckCircle2, XCircle, Loader2, Clock } from "lucide-react"
      ...
      <Card className="bg-card/50 backdrop-blur border-primary/50">
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="font-mono text-lg">PYUSD Faucet</CardTitle>
          </div>
          <CardDescription>Gasless PYUSD token claims</CardDescription>
        </CardHeader>
        <CardContent>
          <Alert className="border-amber-500/50 bg-amber-500/10">
            <AlertDescription className="text-sm font-mono">Please connect your wallet to claim PYUSD</AlertDescription>
          </Alert>
        </CardContent>
      </Card>
    )
  ...
  return (
    <Card className="bg-card/50 backdrop-blur border-primary/50">
      <CardHeader>
        ...
        <CardDescription>Claim PYUSD via gasless signature</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="p-2 rounded-lg bg-muted/50 border border-primary/20">
            ...
            <p className="font-mono font-semibold text-sm">
              {faucet.isLoadingFaucetBalance ? (
                ...
                `${(Number(faucet.faucetBalance) / 1e6).toFixed(2)}`
              )}
            </p>
          </div>
          ...
          <div className="p-2 rounded-lg bg-muted/50 border border-primary/20">
            ...
            <p className="font-mono font-semibold text-sm">
              {faucet.isLoadingClaimAmount ? (
                ...
                `${(Number(faucet.claimAmount) / 1e6).toFixed(2)}`
              )}
            </p>
          </div>
          ...
          <div className="p-2 rounded-lg bg-muted/50 border border-primary/20">
            ...
            <p className="font-mono font-semibold text-sm">{`${(Number(faucet.userPyusdBalance) / 1e6).toFixed(2)}`}</p>
          </div>
        </div>
        ...
        {faucet.isCheckingEligibility ? (
          ...
          <Alert className="border-green-500/50 bg-green-500/10">
            ...
            <AlertDescription className="text-sm font-mono font-bold">✅ You can claim PYUSD now!</AlertDescription>
          </Alert>
        ) : (
          <Alert className="border-amber-500/50 bg-amber-500/10">
